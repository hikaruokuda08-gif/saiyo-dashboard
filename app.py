...
# 1. ページ設定
st.set_page_config(page_title="RECRUIT ANALYTICS", layout="wide")
...
with st.sidebar:
    ...
    # 【ここをスッキリさせました】設定を折りたたみ式に変更
    if uploaded_file is not None:
        with st.expander("🛠 CSV項目設定（カラムマッピング）"):
            st.caption("CSVの列名が自動推測と違う場合のみ変更してください")
            ...
            map_last_name = st.selectbox("姓", all_cols, index=get_idx(["姓", "氏名", "氏"], all_cols))
            map_first_name = st.selectbox("名", ["無し"] + all_cols, index=get_idx(["名"], ["無し"] + all_cols))
            map_b_date = st.selectbox("説明会予約日", all_cols, index=get_idx(["予約日", "説明会"], all_cols))
            map_b_st = st.selectbox("説明会参加", all_cols, index=get_idx(["参加", "出席"], all_cols))
            map_s_st = st.selectbox("選考希望", all_cols, index=get_idx(["希望", "ステータス"], all_cols))
            map_i1_d = st.selectbox("一次日程", all_cols, index=get_idx(["一次", "面接日"], all_cols))
            map_i1_r = st.selectbox("一次結果", all_cols, index=get_idx(["結果", "合否"], all_cols))
            map_n_d = st.selectbox("二次案内日", all_cols, index=get_idx(["案内", "送付"], all_cols))
            map_i2_d = st.selectbox("二次日程", all_cols, index=get_idx(["最終", "二次"], all_cols))
...
if uploaded_file is not None:
    try:
        ...
        with col_sel2:
            if stage == "説明会":
                metric_type = st.selectbox("指標", ["参加率", "欠席率"])
            elif stage == "選考希望":
                metric_type = st.selectbox("指標", ["希望率", "辞退・検討率"])
            else:
                metric_type = st.selectbox("指標", ["面接参加率", "合格率", "辞退率"])
        ...
        if stage == "説明会":
            ...
            num = is_attended.sum() if metric_type == "参加率" else den - is_attended.sum()
        elif stage == "選考希望":
            den = is_attended.sum()
            num = is_wanted.sum() if metric_type == "希望率" else den - is_wanted.sum()
        elif stage == "1次選考":
        ...
        if den > 0:
            val = (num / den) * 100
            st.metric(f"{stage}の{metric_type}", f"{val:.1f}%", f"全体 {den} 名中 {num} 名")
            st.progress(val / 100)
        else:
